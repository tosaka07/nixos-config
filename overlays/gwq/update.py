...
def main():
    ...
    # 最新バージョンを取得
    latest_version = get_latest_release("d-kuro", "gwq")
    ...
    new_hash = prefetch_github("d-kuro", "gwq", f"v{latest_version}")
    print(f"Source hash: {new_hash}")

    # hashes.json を更新
    hashes["version"] = latest_version
    hashes["hash"] = new_hash
    # vendorHash は Go モジュールのビルド時に計算が必要
    # 新バージョンでは vendorHash が変わる可能性があるため、
    # 一旦 fakeHash を設定してビルドエラーから正しい値を取得する必要がある
    # ここでは既存の vendorHash を維持（手動更新が必要な場合あり）
    ...
    print("Note: vendorHash may need manual update if Go dependencies changed.")
    return 0
